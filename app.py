...
def bills():
    ...
    base_sql_doctor = """
        SELECT DISTINCT b.*, p.Name as PatientName 
        FROM Bills b
        JOIN Patients p ON b.PatientID = p.PatientID
...
def doctor_dashboard():
    ...
    # Upcoming appointments - include all future appointments, including cancelled ones for display
    upcoming_appointments = query(
        """
        SELECT a.*, p.Name as PatientName 
        FROM Appointments a 
        JOIN Patients p ON a.PatientID = p.PatientID 
        WHERE a.DoctorID = ? AND a.DateTime > GETDATE() 
        ORDER BY a.DateTime
        """,
        (doctor_id,),
    )
